...
namespace RA
{
    public class ResponseContext
    {
        ...
        public ResponseContext Assert(string ruleName)
        {
            ...
            {
                //Console.WriteLine("{2}\n---------\n{0}\n----\n{1}\n------------------",ruleName,_assertions[ruleName],_parsedContent ?? _content);
            ...
            foreach (var type in parsers.Keys)
            {
                ...
                if (contentType.Contains(type))
                {
                    try
                    {
                        ...
                        //Console.WriteLine(_parsedContent);
                        return;
                    }
                    catch
                    {
        ...
        private void ParseLoad()
        {
            if (_loadResponses.Any())
            {
                ...
                _loadValues.Add(LoadValueTypes.SuccessPercent.Value, (_loadResponses.Count(x => (x.StatusCode == (int)HttpStatusCode.OK)) / _loadResponses.Count)*100);
        ...
        /// </summary>
        /// <returns></returns>
        public ResponseContext WriteAssertions(string name = "", bool writeAssertSchema = true)
        {
            (name.Equals("") ? "assertions" : name).WriteHeader();
            foreach (var assertion in _assertions)
            {
                assertion.WriteTest();
            }
            if(writeAssertSchema)
            {
